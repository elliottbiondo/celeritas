...
{
    detail::UnitIndexer::Vec_Size surface_count(units.size());
    detail::UnitIndexer::Vec_Size cell_count(units.size());
    ...
    : unit_indexer_(make_unit_indexer(input.units))
{
    ...
    {
        // Capture metadata
        for (const auto& sl : u.surfaces.labels)
        {
            surface_labels.push_back(u.label.name + std::string(".") + sl.name);
        }

        for (const auto& v : u.volumes)
        {
            volume_labels.push_back(u.label.name + std::string(".")
                                    + v.label.name);
        }
        bbox_ = u.bbox;
    }
