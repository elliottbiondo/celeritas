 ...
 */
UnitIndexer::UnitIndexer(Vec_Size num_surfaces, Vec_Size num_volumes)
{
    CELER_EXPECT(!num_surfaces.empty());
    CELER_EXPECT(num_surfaces.size() == num_volumes.size());

    d_surfaces = count_to_offset(std::move(num_surfaces));
    d_volumes  = count_to_offset(std::move(num_volumes));

    CELER_ENSURE(d_surfaces.size() >= 2);
    CELER_ENSURE(d_surfaces.size() == d_volumes.size());
    ...
    -> SurfaceId
{
    ...
    CELER_EXPECT(surf < this->local_size(d_surfaces, uni));

    return SurfaceId(d_surfaces[uni.unchecked_get()] + surf.unchecked_get());
}
...
{
    CELER_EXPECT(uni < this->num_universes());
    CELER_EXPECT(volume < this->local_size(d_volumes, uni));

    return VolumeId(d_volumes[uni.unchecked_get()] + volume.unchecked_get());
}
 ...
 */
auto UnitIndexer::local_surface(SurfaceId id) const -> Local_Surface
{
    CELER_EXPECT(id < this->num_surfaces());
    auto iter = this->find_local(d_surfaces, id.unchecked_get());

    UniverseId uni(iter - d_surfaces.begin());
 ...
 * \brief Global-to-local
 */
auto UnitIndexer::local_volume(VolumeId id) const -> Local_Volume
{
    CELER_EXPECT(id < this->num_volumes());
    auto iter = this->find_local(d_volumes, id.unchecked_get());

    UniverseId uni(iter - d_volumes.begin());
 ...
 */
auto UnitIndexer::find_local(const Vec_Size& offsets, size_type id)
    -> Vec_Size::const_iterator
{
 ...
 */
auto UnitIndexer::local_size(const Vec_Size& offsets, UniverseId uni)
    -> size_type
{
