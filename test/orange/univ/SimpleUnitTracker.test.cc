...
class SimpleUnitTrackerTest : public OrangeGeoTestBase
{
  ...
  protected:
    ...
    // Prepare for initialization across a surfa
...
TEST_F(OneVolumeTest, intersect)
{
    ...
    {
        auto state
            = this->make_state({1, 2, 3}, {0, 0, 1}, "one_volume.infinite");
...
TEST_F(OneVolumeTest, safety)
{
    ...
    EXPECT_SOFT_EQ(
        inf,
        tracker.safety({1, 2, 3}, this->find_volume("one_volume.infinite")));
}
...
TEST_F(TwoVolumeTest, initialize)
{
...

    {
        ...
        EXPECT_EQ("two_volumes.inside", this->id_to_label(init.volume));
        EXPECT_FALSE(init.surface);
    }
    {
        // Test rejection
    ...
    }
    {
        ...
        EXPECT_EQ("two_volumes.outside", this->id_to_label(init.volume));
        EXPECT_FALSE(init.surface);
    }
}
...
TEST_F(TwoVolumeTest, cross_boundary)
{
...

    {
        ...
        auto init = tracker.cross_boundary(
            this->make_state_crossing({1.5, 0, 0},
                                      {0, 0, 1},
                                      "two_volumes.inside",
                                      "two_volumes.sphere",
                                      '-'));
        EXPECT_EQ("two_volumes.outside", this->id_to_label(init.volume));
        EXPECT_EQ("two_volumes.sphere", this->id_to_label(init.surface.id()));
...
TEST_F(TwoVolumeTest, intersect)
{
...

    {
        SCOPED_TRACE("Inside");
        auto state
            = this->make_state({0.5, 0, 0}, {0, 0, 1}, "two_volumes.inside");
        auto isect = tracker.intersect(state);
        EXPECT_TRUE(isect);
        ...
        state = this->make_state({0.5, 0, 0}, {1, 0, 0}, "two_volumes.inside");
    ...
    }
    {
        ...
        auto state
            = this->make_state({0, 0, 2.0}, {0, 0, 1}, "two_volumes.outside");
        auto isect = tracker.intersect(state);
        EXPECT_FALSE(isect);

        state = this->make_state({0, 0, 2.0}, {0, 0, -1}, "two_volumes.inside");
        isect = tracker.intersect(state);
        EXPECT_TRUE(isect);
        EXPECT_EQ(SurfaceId{0}, isect.surface.id());
    ...
    }
    {
        ...
        auto state = this->make_state({0, 0, 1.5},
                                      {0, 0, 1},
                                      "two_volumes.outside",
                                      "two_volumes.sphere",
                                      '+');
    ...
    }
    {
        ...
        auto state = this->make_state({0, 1.5, 0},
                                      {0, 1, 0},
                                      "two_volumes.inside",
                                      "two_volumes.sphere",
                                      '-');
...
{
    SimpleUnitTracker tracker(this->params().host_ref(), SimpleUnitId{0});
    VolumeId          outside = this->find_volume("two_volumes.outside");
    VolumeId          inside  = this->find_volume("two_volumes.inside");
...
TEST_F(FiveVolumesTest, initialize)
{
...

    {
        ...
        EXPECT_EQ("global.[EXTERIOR]", this->id_to_label(init.volume));
        EXPECT_FALSE(init.surface);
    }
    {
        ...
        EXPECT_EQ("global.e", this->id_to_label(init.volume));
        EXPECT_FALSE(init.surface);
    }
    {
        ...
        EXPECT_EQ("global.a", this->id_to_label(init.volume));
        EXPECT_FALSE(init.surface);
    }
    {
        ...
        EXPECT_EQ("global.d", this->id_to_label(init.volume));
        EXPECT_FALSE(init.surface);
    }
    {
...
TEST_F(FiveVolumesTest, cross_boundary)
{
...

    {
        ...
        auto init = tracker.cross_boundary(this->make_state_crossing(
            {-0.5, -0.25, 0}, {-1, 0, 0}, "global.e", "global.epsilon.s", '-'));
        EXPECT_EQ("global.c", this->id_to_label(init.volume));
        EXPECT_EQ("global.epsilon.s", this->id_to_label(init.surface.id()));
        EXPECT_EQ(Sense::outside, init.surface.sense());
    }
    {
        SCOPED_TRACE(
        ...
        real_type eps  = 1e-10;
        auto      init = tracker.cross_boundary(this->make_state_crossing(
            {eps, -0.25, 0}, {1, 0, 0}, "global.e", "global.epsilon.s", '-'));
        EXPECT_EQ("global.c", this->id_to_label(init.volume));
        EXPECT_EQ("global.epsilon.s", this->id_to_label(init.surface.id()));
        EXPECT_EQ(Sense::outside, init.surface.sense());
    }
    {
        ...
        auto init = tracker.cross_boundary(
            this->make_state_crossing({-.75 * sqrt_half, 0.75 * sqrt_half, 0},
                                      ...
                                      '-'));
        EXPECT_EQ("global.a", this->id_to_label(init.volume));
        EXPECT_EQ("global.gamma.s", this->id_to_label(init.surface.id()));
        EXPECT_EQ(Sense::outside, init.surface.sense());
    }
    {
    ...
    }
    {
        ...
        auto init = tracker.cross_boundary(this->make_state_crossing(
            {0, 0.75, 0}, {0, 1, 0}, "global.c", "global.gamma.s", '-'));
        EXPECT_EQ("global.d", this->id_to_label(init.volume));
        EXPECT_EQ("global.gamma.s", this->id_to_label(init.surface.id()));
        EXPECT_EQ(Sense::outside, init.surface.sense());

        init = tracker.cross_boundary(this->make_state_crossing(
            {0, 0.75, 0}, {-1, 0, 0}, "global.d", "global.gamma.s", '+'));
        EXPECT_EQ("global.c", this->id_to_label(init.volume));
        EXPECT_EQ("global.gamma.s", this->id_to_label(init.surface.id()));
        EXPECT_EQ(Sense::inside, init.surface.sense());
        ...
        init = tracker.cross_boundary(this->make_state_crossing(
            {0, 0.75, 0}, {-1, 0, 0}, "global.d", "global.alpha.px", '+'));
        EXPECT_EQ("global.a", this->id_to_label(init.volume));
        EXPECT_EQ("global.alpha.px", this->id_to_label(init.surface.id()));
        EXPECT_EQ(Sense::inside, init.surface.sense());
    }
}
...
TEST_F(FiveVolumesTest, intersect)
{
...

    {
        ...
        auto state = this->make_state({-0.75, 0.5, 0}, {1, 0, 0}, "global.a");
        auto isect = tracker.intersect(state);
        EXPECT_TRUE(isect);
        EXPECT_EQ("global.gamma.s", this->id_to_label(isect.surface.id()));
        EXPECT_EQ(Sense::outside, isect.surface.unchecked_sense());
        EXPECT_SOFT_EQ(0.19098300562505255, isect.distance);
    }
    {
        SCOPED_TRACE("skip internal surfaces for d");
        auto state = this->make_state({-2, 0.5, 0}, {1, 1, 0}, "global.d");
        auto isect = tracker.intersect(state);
        EXPECT_TRUE(isect);
        EXPECT_EQ("global.outer.s", this->id_to_label(isect.surface.id()));
        EXPECT_EQ(Sense::inside, isect.surface.unchecked_sense());
...
TEST_F(FiveVolumesTest, safety)
{
    ...
    VolumeId a = this->find_volume("global.a");
    VolumeId d = this->find_volume("global.d");
